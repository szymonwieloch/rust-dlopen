...
use crate::common::{get_fields, has_marker_attr, symbol_name};

const ALLOW_NULL: &str = "dlopen_allow_null";
const TRAIT_NAME: &str = "WrapperApi";

pub fn impl_wrapper_api(ast: &DeriveInput) -> TokenStream2 {
    ...
    for field in fields.named.iter() {
        let _ = field
            .ident
            .as_ref()
            .expect("All fields of structures deriving WrapperAPI need to be identifiable");
        match field.vis {
...
fn normal_field(field: &Field) -> TokenStream2 {
...
fn allow_null_field(field: &Field, ptr: &TypePtr) -> TokenStream2 {
    let field_name = &field.ident;
    let symbol_name = symbol_name(field);
